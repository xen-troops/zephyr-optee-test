...
void *regression_4000_init(void)
{
	printk("Begin Test suite 4000\n");
	level = 15;
	(void)TEEC_InitializeContext(NULL, &xtest_teec_ctx);
	return NULL;
}

void regression_4000_deinit(void *param)
{
	(void)param;
	printk("End Test suite 4000\n");
	TEEC_FinalizeContext(&xtest_teec_ctx);
}

static TEEC_Result ta_crypt_cmd_reset_operation(struct ADBG_Case *c, TEEC_Session *s,
...
static TEEC_Result ta_crypt_cmd_copy_operation(struct ADBG_Case *c,
					       TEEC_Session *s,
...
static TEEC_Result ta_crypt_cmd_digest_update(struct ADBG_Case *c, TEEC_Session *s,
					      TEE_OperationHandle oph,
					      const void *chunk,
					      size_t chunk_size)
{
...
static TEEC_Result ta_crypt_cmd_digest_do_final(struct ADBG_Case *c, TEEC_Session *s,
						TEE_OperationHandle oph,
						const void *chunk,
...
static TEE_Result ta_crypt_cmd_set_operation_key2(struct ADBG_Case *c,
						  TEEC_Session *s,
						  TEE_OperationHandle oph,
...
static TEEC_Result ta_crypt_cmd_mac_init(struct ADBG_Case *c, TEEC_Session *s,
					 TEE_OperationHandle oph,
...
static TEEC_Result ta_crypt_cmd_mac_update(struct ADBG_Case *c, TEEC_Session *s,
					   TEE_OperationHandle oph,
...
static TEEC_Result ta_crypt_cmd_mac_final_compute(struct ADBG_Case *c,
						  TEEC_Session *s,
						  TEE_OperationHandle oph,
						  const void *chunk,
						  size_t chunk_len,
...
static TEEC_Result ta_crypt_cmd_mac_final_compare(struct ADBG_Case *c,
						  TEEC_Session *s,
						  TEE_OperationHandle oph,
						  const void *chunk,
						  size_t chunk_len,
...
static TEEC_Result ta_crypt_cmd_cipher_init(struct ADBG_Case *c, TEEC_Session *s,
					    TEE_OperationHandle oph,
					    const void *iv, size_t iv_len)
{
	TEEC_Result res = TEEC_ERROR_GENERIC;
...
static TEEC_Result ta_crypt_cmd_cipher_update(struct ADBG_Case *c, TEEC_Session *s,
					      TEE_OperationHandle oph,
...
static TEEC_Result ta_crypt_cmd_cipher_do_final(struct ADBG_Case *c,
						TEEC_Session *s,
						TEE_OperationHandle oph,
						const void *src,
						size_t src_len,
...
static TEEC_Result ta_crypt_cmd_random_number_generate(struct ADBG_Case *c,
						       TEEC_Session *s,
						       void *buf,
						       size_t blen)
{
...
static TEEC_Result ta_crypt_cmd_ae_init(struct ADBG_Case *c, TEEC_Session *s,
					TEE_OperationHandle oph,
...
static TEEC_Result ta_crypt_cmd_ae_update_aad(struct ADBG_Case *c, TEEC_Session *s,
					      TEE_OperationHandle oph,
...
static TEEC_Result ta_crypt_cmd_ae_update(struct ADBG_Case *c,
					  TEEC_Session *s,
					  TEE_OperationHandle oph,
					  const void *src,
					  size_t src_len,
...
static TEEC_Result ta_crypt_cmd_ae_encrypt_final(struct ADBG_Case *c,
						 TEEC_Session *s,
						 TEE_OperationHandle oph,
						 const void *src,
...
static TEEC_Result ta_crypt_cmd_ae_decrypt_final(struct ADBG_Case *c,
						 TEEC_Session *s,
						 TEE_OperationHandle oph,
						 const void *src, size_t src_len,
						 void *dst, size_t *dst_len,
...
static TEEC_Result ta_crypt_cmd_asymmetric_operate(struct ADBG_Case *c,
						   TEEC_Session *s,
						   TEE_OperationHandle oph,
						   uint32_t cmd,
...
static TEEC_Result ta_crypt_cmd_asymmetric_encrypt(struct ADBG_Case *c,
						   TEEC_Session *s,
						   TEE_OperationHandle oph,
						   const TEE_Attribute *params,
						   uint32_t paramCount,
...
static TEEC_Result ta_crypt_cmd_asymmetric_decrypt(struct ADBG_Case *c,
						   TEEC_Session *s,
						   TEE_OperationHandle oph,
						   const TEE_Attribute *params,
						   uint32_t paramCount,
...
static TEEC_Result ta_crypt_cmd_asymmetric_sign(struct ADBG_Case *c,
						TEEC_Session *s,
						TEE_OperationHandle oph,
						const TEE_Attribute *params,
						uint32_t paramCount,
...
static TEEC_Result ta_crypt_cmd_asymmetric_verify(struct ADBG_Case *c,
						  TEEC_Session *s,
						  TEE_OperationHandle oph,
						  const TEE_Attribute *params,
						  uint32_t paramCount,
...
static TEEC_Result ta_crypt_cmd_get_object_value_attribute(struct ADBG_Case *c,
							   TEEC_Session *s,
							   TEE_ObjectHandle o,
							   uint32_t attr_id,
							   uint32_t *valuea,
...
static TEEC_Result ta_crypt_cmd_generate_key(struct ADBG_Case *c,
					     TEEC_Session *s,
					     TEE_ObjectHandle o,
					     uint32_t key_size,
					     const TEE_Attribute *params,
...
ZTEST(regression_4000, test_4001)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	size_t n = 0;
	ADBG_STRUCT_DECLARE("Test TEE Internal API hash operations");
	...
	for (n = 0; n < ARRAY_SIZE(hash_cases); n++) {
		...
		BeginSubCase("Hash case %d algo 0x%x",
...
ZTEST(regression_4000, test_4002)
{
	TEEC_Session session = { };
	TEE_OperationHandle op1 = TEE_HANDLE_NULL;
	TEE_OperationHandle op2 = TEE_HANDLE_NULL;
	...
	uint32_t ret_orig = 0;
	size_t n = 0;
	ADBG_STRUCT_DECLARE("Test TEE Internal API MAC operations");
	...
	for (n = 0; n < ARRAY_SIZE(mac_cases); n++) {
		...
		BeginSubCase("MAC case %d algo 0x%x",
...
ZTEST(regression_4000, test_4003)
{
	TEEC_Session session = { };
	...
	uint32_t ret_orig = 0;
	size_t n = 0;
	ADBG_STRUCT_DECLARE("Test TEE Internal API cipher operations");
	...
	for (n = 0; n < ARRAY_SIZE(ciph_cases); n++) {
		...
		BeginSubCase("Cipher case %d algo 0x%x line %d",
...
ZTEST(regression_4000, test_4004)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	uint8_t buf1[45] = { };
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API get random");

	BeginSubCase("TEE get random");
	if (!ADBG_EXPECT_TEEC_SUCCESS(&c,
		xtest_teec_open_session(&session, &crypt_user_ta_uuid, NULL,
			&ret_orig)))
		ADBG_Assert(&c);
...
ZTEST(regression_4000, test_4005)
{
	TEEC_Session session = { };
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	TEE_OperationHandle op2 = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Authenticated Encryption operations");
	...
	for (n = 0; n < ARRAY_SIZE(ae_cases); n++) {
		BeginSubCase("AE case %d algo 0x%x line %d",
...
static bool create_key(struct ADBG_Case *c, TEEC_Session *s,
...
ZTEST(regression_4000, test_4006)
{
	TEEC_Session session = { };
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Asymmetric Cipher operations");
	...
	for (n = 0; n < ARRAY_SIZE(xtest_ac_cases); n++) {
		...
		BeginSubCase("Asym Crypto case %d algo 0x%x line %d",
...
static bool test_keygen_attributes(struct ADBG_Case *c, TEEC_Session *s,
...
static bool test_secret_value(struct ADBG_Case *c, TEEC_Session *s,
			      TEE_ObjectHandle key, uint32_t key_size)
{
...
static bool test_rsa_key_pair(struct ADBG_Case *c, TEEC_Session *s,
			      TEE_ObjectHandle key, uint32_t key_size)
{
	const struct key_attrs attrs[] = {
...
static bool test_ecc_key_pair(struct ADBG_Case *c, TEEC_Session *s,
			      TEE_ObjectHandle key, uint32_t key_size)
{
	const struct key_attrs attrs[] = {
...
static bool test_dh_key_pair(struct ADBG_Case *c, TEEC_Session *s,
...
static bool test_dsa_key_pair(struct ADBG_Case *c, TEEC_Session *s,
			      TEE_ObjectHandle key, uint32_t key_size)
{
	const struct key_attrs attrs[] = {
...
static bool test_x25519_key_pair(struct ADBG_Case *c, TEEC_Session *s,
			      TEE_ObjectHandle key, uint32_t key_size)
{
	const struct key_attrs attrs[] = {
...
static bool generate_and_test_key(struct ADBG_Case *c, TEEC_Session *s,
...
static void keygen_noparams(struct ADBG_Case *c, TEEC_Session *session,
			    ...
			    size_t num_key_types)
{
	...
	for (n = 0; n < num_key_types; n++) {
		...
		BeginSubCase("Generate %s key", key_types[n].name);
		...
		EndSubCase("Generate %s key", key_types[n].name);
	}
}

ZTEST(regression_4000, test_4007_symmetric)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Generate Symmetric key");
...
ZTEST(regression_4000, test_4007_rsa)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	static const struct key_types_noparam key_types[] = {
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Generate RSA key");
...
ZTEST(regression_4000, test_4007_dh)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	size_t n = 0;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Generate DH key");
...
#define XTEST_DH_GK_DATA_SUBPRIME(vect) \
	...
	for (n = 0; n < ARRAY_SIZE(key_types); n++) {
		...
		BeginSubCase("Generate DH key %d bits - Private bits = %d",
		...
		EndSubCase("Generate DH key %d bits - Private bits = %d",
			   key_types[n].key_size,
			   *key_types[n].private_bits);
	}
...
ZTEST(regression_4000, test_4007_dsa)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	size_t n = 0;
...
#define XTEST_DSA_GK_DATA(vect) \
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Generate DSA key");
	...
	for (n = 0; n < ARRAY_SIZE(key_types); n++) {
		...
		BeginSubCase("Generate DSA key %d bits",
			     key_types[n].key_size);
		param_count = 0;
		...
		EndSubCase("Generate DSA key %d bits",
			   key_types[n].key_size);
	}
...
ZTEST(regression_4000, test_4007_ecc)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	size_t n = 0;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Generate ECC key");
	...
	for (n = 0; n < ARRAY_SIZE(key_types); n++) {
		...
		BeginSubCase("Generate %s", key_types[n].name);
		param_count = 0;
		...
		EndSubCase("Generate %s", key_types[n].name);
	}
...
ZTEST(regression_4000, test_4007_x25519)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;

	ADBG_STRUCT_DECLARE("Test TEE Internal API Generate X25519 key");
	...
	BeginSubCase("Generate X25519 key");
	...
	EndSubCase("Generate X25519 key");
out:
	TEEC_CloseSession(&session);
	ADBG_Assert(&c);
}

ZTEST(regression_4000, test_4008)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Derive key");
	...
	BeginSubCase("Derive DH key success");
	...
	if (!ADBG_EXPECT_TEEC_SUCCESS(&c,
		...
		goto out;
out:
	EndSubCase("Derive DH key success");
	TEEC_CloseSession(&session);
	ADBG_Assert(&c);
}

ZTEST(regression_4000, test_4009)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Derive key ECDH");
	...
	for (i = 0; i < ARRAY_SIZE(derive_key_ecdh); i++) {
		...
		BeginSubCase("Derive ECDH key - algo = 0x%x",
			     pt->algo);
		...
		EndSubCase("Derive ECDH key - algo = 0x%x",
			   pt->algo);
	}
...
out:
	EndSubCase("Derive ECDH key - algo = 0x%x", pt->algo);
...
ZTEST(regression_4000, test_4010)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API create transient object (negative)");
...
ZTEST(regression_4000, test_4011)
{
	TEEC_Session s = { };
	size_t key_size = 512;
	TEE_ObjectHandle key = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Bleichenbacher attack (negative)");
...
ZTEST(regression_4000, test_4012)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	TEEC_Operation op = TEEC_OPERATION_INITIALIZER;
	...
	ADBG_STRUCT_DECLARE("Test seeding RNG entropy");
...
ZTEST(regression_4000, test_4013)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	TEEC_Operation op = TEEC_OPERATION_INITIALIZER;
	...
	ADBG_STRUCT_DECLARE("Test generation of device unique TA keys");
...
ZTEST(regression_4000, test_4014)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test SM2 KEP (key derivation)");
	...
	EndSubCase("Initiator side");

	BeginSubCase("Responder side");
...
ZTEST(regression_4000, test_4015)
{
	TEEC_Session session = { };
	uint32_t ret_orig = 0;
	TEE_OperationHandle op = TEE_HANDLE_NULL;
	...
	ADBG_STRUCT_DECLARE("Test TEE Internal API Derive key X25519");
	...
	EndSubCase("%s", case_str);

	strncpy(case_str, "Bob side computes shared secret",
		sizeof(case_str) - 1);

	Do_ADBG_BeginSubCase("%s", case_str);
...
ZTEST_SUITE(regression_4000, NULL, regression_4000_init, NULL, NULL,
	    regression_4000_deinit);
