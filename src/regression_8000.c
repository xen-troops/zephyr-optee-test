...
void *regression_8000_init(void)
{
	printk("Begin Test suite 8000\n");
	level = 15;
...
ZTEST(regression_8000, test_8001)
{
	ADBG_STRUCT_DECLARE("Test TEE Internal API key derivation extensions");

	xtest_tee_test_8001(&c);
	ADBG_Assert(&c);
}
...
ZTEST(regression_8000, test_8002)
{
	ADBG_STRUCT_DECLARE("Secure Storage Key Manager API Self Test");

	xtest_tee_test_8002(&c);
	ADBG_Assert(&c);
}

ZTEST_SUITE(regression_8000, NULL, regression_8000_init, NULL, NULL,
	    regression_8000_deinit);
