...
#define CFG_TA_MBEDTLS
#define CFG_TA_MBEDTLS_SELF_TEST
...
void *regression_8100_init(void)
{
	printk("Begin Test suite 8100\n");
	level = 15;
	(void)TEEC_InitializeContext(NULL, &xtest_teec_ctx);
	return NULL;
}

void regression_8100_deinit(void *param)
{
	(void)param;
	printk("End Test suite 8100\n");
...
ZTEST(regression_8100, test_8101)
{
	ADBG_STRUCT_DECLARE("TA mbedTLS self tests");

	test_8101(&c);
	ADBG_Assert(&c);
}

__attribute__ ((format (printf, 2, 3))) static int myasprintf(char **strp, const char *fmt, ...)
{
	char *str = NULL;
	int rc = 0;
	va_list ap;
...
ZTEST(regression_8100, test_8102)
{
	ADBG_STRUCT_DECLARE("TA mbedTLS test cert chain");
...
ZTEST(regression_8100, test_8103)
{
	ADBG_STRUCT_DECLARE("TA mbedTLS process certificate request");
...
ZTEST_SUITE(regression_8100, NULL, regression_8100_init, NULL, NULL,
	    regression_8100_deinit);
